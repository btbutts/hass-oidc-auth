...
def compute_allowed_signing_algs(
    ...
    if configured_alg:
        allowed_algs = [configured_alg]
        if configured_alg not in supported_algs:
            logger.warning(
                "Configured id_token_signing_alg '%s' not in OP supported algorithms %s. Proceeding anyway.",
                configured_alg, supported_algs
            )
    else:
